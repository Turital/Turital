...
def show_zodiac_bottons_on(message):
    ...
    BoyGoW.sand_message(message.from_user.id, 'ВЫбери свой знак зодиака, МАЛЬЧИК!', reply_makeup=keyboard)
...
def get_text_message(message):
    if message.text == r'\hello':
        ...
        BoyGoW.send+message(message.from_user.id, 'Напиши \\hello МАЛЬЧИК!!!')
    else:
...
def callback_worker(call):
    if call_data == 'zodiac':
        ...
        BoyGoW.sand_message(call.message.chat.id, msg)
